...
module.exports = {
    ...
    generateAcctTypes: function(collectionName, callback){
        var db   = mongoUtil.getDb();
        var coll = db.collection(collectionName);
        coll.distinct('val.dim.acct_type', function(err, doc){
            if(err){
                callback(err, null);
            } else {
                callback(null, doc);
    ...
    generateAcctTypeDataPoints: function(collectionName, params, callback){
        var db      = mongoUtil.getDb();
        var coll    = db.collection(collectionName);
        var from    = new Date(params.range.from);
        var to      = new Date(params.range.to);
        ...
        for(var i = 0; i < targets.length; i++){
            if(targets[i].type === 'timeserie'){
                search.$or.push({$eq: ['$$val.dim.acct_type', targets[i].target]});
            }
        }
        ...
        coll.aggregate([
            ...
            {$project:{_id: 0, date: 1, val:{$filter:{input: '$val', as: 'val', cond: search}}}},
            {$unwind: '$val'},
            {$project: {date: 1, acct_type: '$val.dim.acct_type', value: '$val.value'}},
            {$group: {_id: {date: '$date', acct_type: '$acct_type'}, avgValue: {$avg: '$value'}}},
            {$project: {date: '$_id.date', acct_type: '$_id.acct_type', avgValue: '$avgValue', _id: 0}},
            {$group: {_id: '$acct_type', results: {$addToSet: {avgValue: '$avgValue', date: '$date'}}}},
            {$project: {target: '$_id', datapoints: '$results', _id: 0}},
            {$limit: limit}
        ], function(err, results){
            if(err){
